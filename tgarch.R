      ...
      for (garch_p in 1:2) {
        for (garch_q in 1:2) {
          ...
          for (dist in c("norm", "std", "ged")) {
            spec <- ugarchspec(
              variance.model = list(model = "fGARCH", submodel = "TGARCH", garchOrder = c(garch_p, garch_q)),  # Use TGARCH as a submodel of fGARCH
...
# Save model summary
sink(file.path("results4", "tgarch_fit_summary.txt"))  # Updated file name
summary(fit)
sink()
...
save_plot(function() {
  par(mfrow = c(2, 2))
  plot(residuals, type = "l", main = "Residuals from TGARCH Model", xlab = "Time", ylab = "Residuals")
...
}, "tgarch_residuals.png")  # Updated file name
...
save_text_output(formatted_forecast_results, "tgarch_forecast_results.txt")  # Updated file name
...
# Forecasted values (log returns) for tGARCH
forecasted_returns <- c(-0.000013, 0.000003, 0.000005, 0.000007, 0.000009, 
                        0.000010, 0.000012, 0.000013, 0.000014, 0.000015)

# Forecasted volatility for tGARCH
forecasted_volatility <- c(0.000133, 0.000152, 0.000170, 0.000189, 0.000207, 
                           0.000225, 0.000244, 0.000262, 0.000281, 0.000299)
