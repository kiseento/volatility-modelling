      ...
      for (garch_p in 1:2) {
        for (garch_q in 1:2) {
          ...
          for (dist in c("norm", "std", "ged")) {
            spec <- ugarchspec(
              variance.model = list(model = "eGARCH", garchOrder = c(garch_p, garch_q)),  # Changed to EGARCH
...
spec <- ugarchspec(
  variance.model = list(model = "eGARCH", garchOrder = best_model$garch_order),  # Changed to EGARCH
...
# Save model summary
sink(file.path("results2", "egarch_fit_summary.txt"))  # Updated file name
summary(fit)
sink()
...
save_plot(function() {
  par(mfrow = c(2, 2))
  plot(residuals, type = "l", main = "Residuals from EGARCH Model", xlab = "Time", ylab = "Residuals")
...
}, "egarch_residuals.png")  # Updated file name
...
save_text_output(formatted_forecast_results, "egarch_forecast_results.txt")  # Updated file name
...
# Save forecast plot
save_plot(function() plot(forecast, which = 3), "egarch_forecast.png")  # Updated file name

# ✅ Final message
cat("All results and plots saved in the 'results2' folder.\n")
...
# Forecasted values (log returns) for eGARCH
forecasted_returns <- c(-0.000003, 0.000007, 0.000007, 0.000008, 0.000008, 
                        0.000009, 0.000009, 0.000010, 0.000010, 0.000011)

# Forecasted volatility for eGARCH
forecasted_volatility <- c(0.000090, 0.000080, 0.000080, 0.000080, 0.000081, 
                           0.000081, 0.000081, 0.000082, 0.000082, 0.000082)
