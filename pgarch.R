...
auto_garch <- function(returns) {
  ...
  best_dist <- "ged"  # Lock to Generalized Error Distribution (GED)
  ...
  for (p in 0:2) {
    for (q in 0:2) {
      ...
      for (garch_p in 1:2) {
        for (garch_q in 1:2) {
          # Only use GED distribution
          spec <- ugarchspec(
            variance.model = list(model = "apARCH", garchOrder = c(garch_p, garch_q)),  # Use apARCH for PGARCH
            mean.model = list(armaOrder = c(p, q), include.mean = TRUE),
            distribution.model = best_dist  # Lock to GED
          )
          fit <- tryCatch({
...
spec <- ugarchspec(
  variance.model = list(model = "apARCH", garchOrder = best_model$garch_order),  # Use apARCH for PGARCH
  mean.model = list(armaOrder = best_model$arma_order, include.mean = TRUE),
  distribution.model = best_model$dist  # Lock to GED
)
fit <- ugarchfit(spec, returns)

# Save model summary
sink(file.path("results3", "pgarch_fit_summary.txt"))  # Updated file name
summary(fit)
sink()
...
save_plot(function() {
  par(mfrow = c(2, 2))
  plot(residuals, type = "l", main = "Residuals from PGARCH Model", xlab = "Time", ylab = "Residuals")
...
}, "pgarch_residuals.png")  # Updated file name
...
save_text_output(formatted_forecast_results, "pgarch_forecast_results.txt")  # Updated file name
...
# Save forecast plot
save_plot(function() plot(forecast, which = 3), "pgarch_forecast.png")  # Updated file name

# Final message
cat("All results and plots saved in the 'results3' folder.\n")

# Validation
actual_rates <- c(129.2927, 129.3060, 129.2894, 129.3480, 129.3805, 
                  129.4176, 129.4688, 129.4860, 129.4843, 129.5063, 129.4729)

# Forecasted values (log returns) for pGARCH
forecasted_returns <- c(-0.000015, -0.000001, -0.000001, -0.000001, -0.000001, 
                        -0.000001, -0.000001, -0.000001, -0.000001, -0.000001)

# Forecasted volatility for pGARCH
forecasted_volatility <- c(0.000119, 0.000125, 0.000138, 0.000150, 0.000162, 
                           0.000174, 0.000186, 0.000197, 0.000209, 0.000221)
